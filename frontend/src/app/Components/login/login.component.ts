...
export class LoginComponent {
  ...
  LoginUser() {
    //find user from registered users
    ...
    if (this.loginUser.Password == null) {
      ...
      let passOfId: any = this.allUsers
        .filter((x) => x.id == this.user)
        .find((x) => x.id == this.user)?.password;
    ...
    //check approval
    let userbe: any = this.allUsers.find(
      (u) => u.email == this.loginUser.Email
    )?.approved;
